...
def train(args):
    ...
    for epoch in range(0, args.epochs):
        ...
        for iter, data in train_pbar:
            ...
            for i in range(0, args.train_ratio * 10):
                g_optimizer.zero_grad()
                z = Variable(Tensor(np.random.normal(0, 1, (imgs.size(0), args.latent_dimension)))).view(-1, args.latent_dimension, 1, 1)
        ...
        if epoch != 0 and epoch % args.test_term == 0:
            generator.eval()
            latent_vector = Variable(Tensor(np.random.normal(0, 1, (args.test_batch_size, args.latent_dimension, 1, 1))))
